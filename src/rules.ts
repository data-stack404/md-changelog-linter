import { ParsedChangelog, LintDiagnostic } from './types';
...
function isValidDate(dateStr: string): boolean {
  if (!DATE_FORMAT_RE.test(dateStr)) return false;
...
export function runRules(changelog: ParsedChangelog): LintDiagnostic[] {
  ...
  const unreleased = changelog.versions.find(
    v => v.label.toLowerCase() === 'unreleased',
  );
  if (!unreleased) {
    diagnostics.push({
      line: 1,
  ...
  const releases = changelog.versions.filter(
    v => v.label.toLowerCase() !== 'unreleased',
  );
